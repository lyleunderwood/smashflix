...
return function()
  return util.deepcopy({
    ...
    behavior = {
      ...
      startHealth = PRIEST_HEALTH,

      currentHealth = PRIEST_HEALTH,
      ...
      doWalkState = function(self)
        ...
        if not self.walkTimer then
          ...
          self.walkTimer:setSpan(PRIEST_WALK_TIME)
      ...
      doStopState = function(self)
        ...
        if not self.stopTimer then
          ...
          self.stopTimer:setSpan(PRIEST_STOP_DELAY)
        ...
        if not self.fireTimer then
          ...
          self.fireTimer:setSpan(PRIEST_FIRE_DELAY)
      ...
      updateMovement = function(self)
        ...
        local length = (time - self.lastFrameTime) * PRIEST_BASE_SPEED
      ...
      buildMovementTransform = function(self, length)
        ...
        local angular = math.sqrt(PRIEST_BASE_SPEED * PRIEST_BASE_SPEED / 2)
        ...
        elseif mov.up and mov.left then
          self.rig.body:setLinearVelocity(-angular, angular)
        elseif mov.up then
          self.rig.body:setLinearVelocity(0, PRIEST_BASE_SPEED)
        ...
        elseif mov.right then
          self.rig.body:setLinearVelocity(PRIEST_BASE_SPEED, 0)
