...
return util.deepcopy({
  spritesheetName = "spritesheets/ninjastar",
  pos = {x = -0, y = -0},
  angle = 0,
  size = {w = 5, h = 5},
  behavior = {
    ...
    start = function(self, rig)
      ...
      rig.fixture:setFilter(COL_ENEMY_BULLET, util.bor(COL_WALL, COL_PC))
      rig.fixture.behavior = self

      local behavior = self
      rig.fixture:setCollisionHandler(function(phase, bullet, other, arbiter)
        behavior:impact(other)
      end, MOAIBox2DArbiter.BEGIN, util.bor(COL_WALL, COL_PC))
    ...
    updateMovement = function(self, length)
      ...
      local angular = math.sqrt(NINJA_STAR_SPEED * NINJA_STAR_SPEED / 2)
      ...
      elseif mov.up and mov.left then
        self.rig.body:setLinearVelocity(-angular, angular)
      elseif mov.up then
        self.rig.body:setLinearVelocity(0, NINJA_STAR_SPEED)
      ...
      elseif mov.right then
        self.rig.body:setLinearVelocity(NINJA_STAR_SPEED, 0)
